...
def save_uploaded_file(file_bytes: bytes, is_complete: bool) -> str:
    os.makedirs("uploads", exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
    status = "complete" if is_complete else "partial"
    filename = f"syslog_{status}_{timestamp}.log"
...
def handle_upload(sock: socket.socket, header_line: str) -> str:
    try:
        ...
        is_complete = len(file_bytes) == file_size

        saved_path = save_uploaded_file(file_bytes, is_complete)

        if not is_complete:
            return (
                f"WARNING: Incomplete file received. "
                f"Expected {file_size} bytes, got {len(file_bytes)} bytes. "
                f"Partial file saved to {saved_path}"
            )
        ...
        if not parsed_logs:
            return f"ERROR: No valid syslog entries found in file. File saved to {saved_path}"
        ...
        return (
            f"SUCCESS: File received and saved to {saved_path}. "
            f"{len(parsed_logs)} syslog entries parsed and indexed. "
